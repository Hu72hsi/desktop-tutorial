...
#sintax = read.table(sintax_dir, header=F, row.names=1, sep="\t", fill = TRUE, comment.char="")
...
# 筛选非细菌、古菌序列
# grep返回位置，grepl返回逻辑值
idx = grepl("Fungi|Spike", sintax$V4, perl = T)
...
suppressWarnings(write.table(paste("\nSampleID\t",  sep=""), file=opts$stat, append = T, sep="\t", quote=F, row.names=F, col.names=F, eol = ""))
