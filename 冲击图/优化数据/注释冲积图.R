...
p=ggplot(data_frame,aes(x = sample_id,y = value*100,fill = Genus,stratum = Genus,alluvium = Genus))+
  ...
  labs(x='samples',y='Relative Abundance (%)',title="真菌组成变化情况")+
  #设置xy轴标识，图例默认为Genus
...
ggsave("沙变土真菌属水平组成变化.pdf",p,device=cairo_pdf,width = 10,height = 8)

df <- read.table(file = "ITS.sum_p.txt", header = TRUE,row.names = 1, sep = "\t")
#导入注释数据
df<-df[,c(1:10)]
data_nrom = df
for(j in 1:10){
...
ggsave("沙变土真菌门水平组成变化.pdf",p,device=cairo_pdf,width = 10,height = 8)
