...
do
    ...
    #上面拆分按需要决定加不加
    cutadapt -u 29 -a ATTAGAWACCCVHGTAGTCC -e 0.15 -m 50 --too-short-output $outdir/$sample/${sample}.too.short_f.fq -o $outdir/$sample/${sample}.trimmed_f.fq.gz --untrimmed-output $outdir/$sample/${sample}.untrimmed_f.fq.gz $rootdir/*/forward/${sample}.fq.gz >> $outdir/$sample/${sample}.rmprimer.log
    cutadapt -u 30 -a TTACCGCGGCKGCTGGCAC -e 0.15 -m 50 --too-short-output $outdir/$sample/${sample}.too.short_r.fq -o $outdir/$sample/${sample}.trimmed_r.fq.gz --untrimmed-output $outdir/$sample/${sample}.untrimmed_r.fq.gz $rootdir/*/reverse/${sample}.fq.gz >> $outdir/$sample/${sample}.rmprimer.log
    #上面是16S
    #cutadapt -u 32 -a GCATCGATGAAGAACGCAGC -e 0.15 -m 50 --too-short-output $outdir/$sample/${sample}.too.short_f.fq -o $outdir/$sample/${sample}.trimmed_f.fq.gz --untrimmed-output $outdir/$sample/${sample}.untrimmed_f.fq.gz $rootdir/*/forward/${sample}.fq.gz >> $outdir/$sample/${sample}.rmprimer.log
    #cutadapt -u 30 -a TTACTTCCTCTAAATGACCAAG -e 0.15 -m 50 --too-short-output $outdir/$sample/${sample}.too.short_r.fq -o $outdir/$sample/${sample}.trimmed_r.fq.gz --untrimmed-output $outdir/$sample/${sample}.untrimmed_r.fq.gz $rootdir/*/reverse/${sample}.fq.gz >> $outdir/$sample/${sample}.rmprimer.log
    #ITS
    #上面剪切,-u是引物加barcode长度，-a是引物序列，引物中的兼并碱基自己查，这两个根据自己的数据来，后面-e是误差，-m是剪切后的序列最小要大于50(一般设置为50),后面生成log文件部分可以删除
...
#双端合并不用运行
    ...
    if [ ! -d $outdir/fastQC_multiqc ];then mkdir $outdir/fastQC_multiqc ; fi
    /hwfssz1/ST_EARTH/P18Z10200N0112/USER/luoxinyue/software/fastqc $outdir/$sample/${sample}.filtered.fq.gz -o $outdir/fastQC_multiqc
    ...
    then
        ...
        rm $outdir/$sample/${sample}.r.fq.gz $outdir/$sample/${sample}.filtered.fq 
done
if [ ! -d $outdir/fastQC_multiqc/multiqc ];then mkdir $outdir/fastQC_multiqc/multiqc ; fi
/jdfssz1/ST_EARTH/P18Z10200N0112/wenjialing/softwares/miniconda3/bin/multiqc $outdir/fastQC_multiqc -o $outdir/fastQC_multiqc/multiqc
#multiqc（可对fastQC结果进行整合）+fastqc结果报告存放路径+multiqc报告输出路径
...
utdir/*/*.sub50000.fa > $rootdir/16S.all.sub50000.fa
